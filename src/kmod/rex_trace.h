...
TRACE_EVENT(rex_match,
	TP_PROTO(struct rex_scan_attr *ctx),

	TP_ARGS(ctx),

	TP_STRUCT__entry(
		__field(__u32, database_id)
		__field(__u32, event_index)
		__field_struct(struct rex_event, event)
	),

	TP_fast_assign(
		__entry->database_id = ctx->database_id;
		__entry->event_index = ctx->nr_events;
		__entry->event = ctx->last_event;
	),

	TP_printk("regex=%u/%u at [%llu, %llu]",
		  __entry->database_id, __entry->event.expression,
		  __entry->event.from, __entry->event.to)
);
